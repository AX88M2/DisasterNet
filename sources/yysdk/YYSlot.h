...
#define SLOT_USE_MASK
//#define CREATE_NEW_OBJECTS_OUTSIDE_GC_WINDOW
...
#ifdef SLOT_USE_MASK
		m_usageMaskChunks = ((_size + 31) >> 5);
		m_usageMask = (uint32*)YYAlloc(m_usageMaskChunks * sizeof(uint32));
		memset(m_usageMask, 0, m_usageMaskChunks * sizeof(uint32));
#endif
	} // end YYSlot
...
#ifdef SLOT_USE_MASK
			int currMaskChunks = m_usageMaskChunks;
			m_usageMaskChunks = ((newsize + 31) >> 5);
			m_usageMask = (uint32*)YYRealloc(m_usageMask, m_usageMaskChunks * sizeof(uint32));
			memset(m_usageMask + currMaskChunks, 0, sizeof(uint32) * (m_usageMaskChunks - currMaskChunks));
#endif
			m_nCapacity = newsize;
		} // end if
...
#ifdef CREATE_NEW_OBJECTS_OUTSIDE_GC_WINDOW
...
#ifdef SLOT_USE_MASK
			m_usageMask[_slot >> 5] &= ~(1 << (_slot & 31));
#endif
			--m_nCount;
		} // end if
	} // end FreeSlot
...
#ifdef SLOT_USE_MASK
	void SetSlotBit(int _slot)
	{
		m_usageMask[_slot >> 5] |= (1 << (_slot & 31));
	}

	void ClearSlotBit(int _slot)
	{
		m_usageMask[_slot >> 5] &= ~(1 << (_slot & 31));
	}

	uint32 GetSlotBit(int _slot)
	{
		return m_usageMask[_slot >> 5] & (1 << (_slot & 31));
	}
#endif
